...
function callNode(args, callback) {
	...
	fork(`${__dirname}/target/node/node`, args, defaults)
	.on('close', callback);
}

gulp.task('config-js-modules', function(done) {
	callNode([
		 ...
		 `--moduleRoot resourcesDir`,
		 `${__dirname}/target/classes/META-INF`
	], done);
});

gulp.task('transpile-js', function(done) {
	callNode([
