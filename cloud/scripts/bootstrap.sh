...
function download_and_extract_files {
	...
	if [ ! -n "${json}" ]
	then
		echo "Error: Could not get metadata from gs://${bucket_name}/${prefix}/"

		exit 1
	fi

	local latest_path=$(jq --raw-output '.items | sort_by(.updated) | last | .name' <<< "${json}")

    if [ "${latest_path}" == "null" ] || [ -z "${latest_path}" ]
	then
        echo "Error: Could not find any files in gs://${bucket_name}/${prefix}/"

        exit 1
    fi
	
    local temp_dir=$(mktemp --directory -t liferay-bootstrap.XXXXXX)
...
function main {
	...
	local config_file=$(get_config_file "${1:-}")

	local provider=$(get_provider "${config_file}")

	local extracted_dir=$(download_and_extract_files "${provider}")
 
    trap 'rm --force --recursive "${extracted_dir}"' EXIT

    "${extracted_dir}/setup_${provider}" "${@}"
}
