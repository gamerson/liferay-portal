...
public class K8sAgentImpl implements K8sAgent {
	...
	protected void deactivate() {
		if (_log.isDebugEnabled()) {
			_log.debug("Deactivating " + K8sAgentConstants.AGENT_NAME);
		}
		...
		if (_log.isDebugEnabled()) {
			_log.debug("Deactivated " + K8sAgentConstants.AGENT_NAME);
		}
	}
	...
	private void _add(ConfigMap configMap) {
		...
		for (Map.Entry<String, String> entry : entrySet) {
			...
			try {
				if (configName.endsWith(K8sAgentConstants.FILE_EXT)) {
					_processConfigMapConfigFileEntry(
						configName,
						_fromStringContent(configName, entry.getValue()),
						configMap.getMetadata());
				}
				else if (configName.endsWith(K8sAgentConstants.FILE_JSON_EXT)) {
	...
	private void _delete(ConfigMap configMap) {
		...
		String configurationFilter = StringBundler.concat(
			StringPool.OPEN_PARENTHESIS, K8sAgentConstants.K8S_CONFIG_UID,
	...
	private Configuration _findExistingConfiguration(String fileName)
		...
		Configuration[] configurations = _configurationAdmin.listConfigurations(
			StringBundler.concat(
				StringPool.OPEN_PARENTHESIS, K8sAgentConstants.K8S_CONFIG_KEY,
	...
	private String[] _parsePid(String path) {
		...
		if (path.endsWith(K8sAgentConstants.FILE_EXT)) {
			pid = path.substring(
				0, path.length() - K8sAgentConstants.FILE_EXT.length());
		}
		else if (path.endsWith(K8sAgentConstants.FILE_JSON_EXT)) {
			pid = path.substring(
				0, path.length() - K8sAgentConstants.FILE_JSON_EXT.length());
		}
	...
	private void _processConfigMapConfigFileEntry(
		...
		if (configuration == null) {
			...
			String existingResourceVersion = (String)properties.get(
				K8sAgentConstants.K8S_CONFIG_RESOURCE_VERSION);
		...
		for (K8sConfigurationPropertiesMutator
				k8sConfigurationPropertiesMutator :
					_k8sConfiguratPropertiesMutators) {

			k8sConfigurationPropertiesMutator.mutateConfigurationProperties(
				metadata.getAnnotations(), metadata.getLabels(), dictionary);
		}

		dictionary.put(K8sAgentConstants.K8S_CONFIG_KEY, configName);
		dictionary.put(K8sAgentConstants.K8S_CONFIG_UID, metadata.getUid());
		dictionary.put(
			K8sAgentConstants.K8S_CONFIG_RESOURCE_VERSION, resourceVersion);
	...
	private void _update(ConfigMap oldConfigMap, ConfigMap newConfigMap) {
					...
					if (configName.endsWith(K8sAgentConstants.FILE_EXT)) {
						_processConfigMapConfigFileEntry(
							...
							metadata);
								...
								K8sAgentConstants.FILE_JSON_EXT)) {
		...
		String configurationFilter = StringBundler.concat(
			...
			K8sAgentConstants.K8S_CONFIG_RESOURCE_VERSION, StringPool.EQUAL,
			oldMetadata.getResourceVersion(), StringPool.CLOSE_PARENTHESIS,
	...
	private final List<K8sConfigurationPropertiesMutator>
		_k8sConfiguratPropertiesMutators;
