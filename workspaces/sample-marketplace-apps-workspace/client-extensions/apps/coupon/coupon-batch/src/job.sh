...
# OAUTH2_APPLICATION_NAME  sets the oauth application to use
...
echo "########################"
if [ "$OAUTH2_APPLICATION_NAME" == "" ];then
	cat <<EOF
No OAuth2 Application was selected for JOB processing!

Please set the environment variable OAUTH2_APPLICATION_NAME in your
...
EOF
	...
	echo "OAUTH2_APPLICATION_NAME = ${OAUTH2_APPLICATION_NAME}"
fi
...
OAUTH2_CLIENTID=$(cat /etc/liferay/lxc/ext-init-metadata/${OAUTH2_APPLICATION_NAME}.oauth2.headless.server.client.id)
OAUTH2_SECRET=$(cat /etc/liferay/lxc/ext-init-metadata/${OAUTH2_APPLICATION_NAME}.oauth2.headless.server.client.secret)
