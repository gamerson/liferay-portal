 ...
 */
@RequestMapping("/external/threads/count")
@RestController
public class ExternalThreadsCountRestController extends BaseRestController {

	@Autowired
	private LiferayOAuth2AccessTokenManager _liferayOAuth2AccessTokenManager;

	@GetMapping
	public ResponseEntity<String> get(@AuthenticationPrincipal Jwt jwt) throws Exception {
		log(jwt, _log);

		String authorization = _liferayOAuth2AccessTokenManager.getAuthorization("external-liferay-oauth-headless-server");

		SiteResource siteResource = SiteResource.builder(
		).header(
			"Authorization", authorization
		).endpoint(
				_externalLiferayServerUrl
		).build();
		...
		return new ResponseEntity<>(Integer.toString(messageBoardThreads.size()), HttpStatus.OK);
	}

	private static final Log _log = LogFactory.getLog(
		ExternalThreadsCountRestController.class);

	@Value("${external.liferay.oauth.headless.server.oauth2.headless.server.home.page.uri}")
	private URL _externalLiferayServerUrl;
